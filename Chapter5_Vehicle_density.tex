\chapter{Vehicle density estimation in congested environments}
...
\subsection{Classical traffic estimation} \label{sssec:ClassicalTraffic}
...
\begin{table}[H]
\centering
\caption[UAS specifications]{UAS specifications (DJI, 2017).}
\label{tb:p3p-specs}
\begin{tabular}{@{}ll@{}}
\hline
...
\subsection{Measuring gap distances}
...
The results of the distance measurements from models generated at different altitudes and corrected using the Haversine method in Equation \ref{eq:distHaversine} are shown in Table \ref{tab:uascalibrate}. The results show that even at the higher operational altitudes, the MAE was only 4.1 cm, or for the smallest measurement (Item 5), a possible error of 8.5\%. This error was considered to be the worst-case. As a result of this test, no correction to the distance measured was applied.
...
Data collection began after traffic had stacked, usually 20-30 seconds into the red light period, and lasted for about 45 seconds. The UAS flew on the inside shoulder to prevent direct overflight of vehicles. Data collection consisted of two passes: the first pass collected images perpendicular to traffic, and the 2nd collected images at a 45 degree offset to the traffic to provide necessary angles for processing as shown in Figure \ref{fig:mission}.
...
\begin{table}[H]
\centering
\caption[Results of z-test for proportions showing p-values for flight pairs for vehicle classes]{Results of z-test for proportions showing p-values for flight pairs for vehicle classes. Failed tests are shown in bold.}
\label{tab:ztestprop}
\begin{tabular}{@{}lcccc@{}}
\toprule
...
\caption{Distribution of fitted LogLogistic and LogNormal curves.}
\label{fig:lognormal}
\end{figure}
